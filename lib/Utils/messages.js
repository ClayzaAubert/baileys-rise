...
function generateWatermarkCoordinates(position = 'center') {
    const coordinates = {
        center: [
            { x: 0, y: 0 },
            { x: 50, y: 0 },
...
const prepareWAMessageMedia = async (message, options) => {
    ...
    const uploadData = {
        ...
        ...(message.annotations ? {
            ...
            annotations: [
                {
                    polygonVertices: generateWatermarkCoordinates('center'),
                    newsletter: {
                        newsletterJid: "120363202967627479@newsletter",
                        serverMessageId: 0,
                        newsletterName: "Maelyn Group | Notify",
                        contentType: "UPDATE",
                    }
                }
            ]
    ...
    const [{ mediaUrl, directPath, handle }] = await Promise.all([
        ...
        if (didSaveToTmpPath && bodyPath) {
            // tambahkan delay untuk memastikan file tidak lagi dipakai
            ...
            try {
                ...
                if (err.code === 'EBUSY') {
                    logger?.warn('Gagal hapus tmp file: sedang digunakan')
                } else {
                    logger?.error('Gagal hapus tmp file:', err)
                }
            }
        }
    }
