...
const makeNewsletterSocket = (config) => {
    ...
    return {
        ...
        newsletterMetadataByJid: async (jid, role = 'GUEST') => {
    try {
        // Extract the newsletter ID from JID
        const newsletterId = jid.split('@')[0];
        
        // Try different approaches to get metadata
        let result;
        
        // Method 1: Use the original newsletterMetadata function with JID type
        try {
            result = await newsletterWMexQuery(undefined, Types_1.QueryIds.METADATA, {
                input: {
                    key: newsletterId,
                    type: 'JID',
                    view_role: role
            ...
            const resultContent = WABinary_1.getBinaryNodeChild(result, 'result')?.content?.toString();
            if (resultContent) {
                const parsed = JSON.parse(resultContent);
                if (parsed.data && parsed.data[Types_1.XWAPaths.NEWSLETTER]) {
                    return extractNewsletterMetadata(result);
                }
            }
        } catch (error) {
        ...
        try {
            ...
            console.log('Method 2 failed, trying method 3:', error.message);
        }
        
        // Method 3: Try using the full JID as key
        try {
            result = await newsletterWMexQuery(undefined, Types_1.QueryIds.METADATA, {
                input: {
                    key: jid,
        ...
        throw new Error(`Unable to fetch newsletter metadata for JID: ${jid}`);
        
    } catch (error) {
        console.error('Error in newsletterMetadataByJid:', error);
        throw error;
    }
},
        newsletterAdminCount: async (jid) => {
            const result = await newsletterWMexQuery(jid, Types_1.ueryIds.ADMIN_COUNT)
